...
export class HomeComponent implements OnInit {
  ...
  fetchServiceData() {
    ...
    this.httpClient.get(`/api/servicos/filtrar/data/${this.currentData}`, { headers })
      .subscribe(
        (response) => {
  ...
  fetchClient(idCliente: string) {
    const url = `/api/clientes/id/${idCliente}`;
  ...
  fetchService(idServico: string) {
    const url = `/api/servicos/id/${idServico}`; // Substitua pelo seu URL de destino
    // Ler o token do LocalStorage
    const token = localStorage.getItem('authToken');
  ...
  fetchServiceById() {
    ...
    this.httpClient.get(`/api/servicos/filtrar/id/${this.serviceNumber}`, { headers }).subscribe((response) => {
      this.responseData = response;
  ...
  fetchServiceByStatus() {
    ...
    let statusFilterUrl = '/api/servicos/filtrar/status/';
  ...
  fetchServiceByType(event: any) {
    ...
    if (serviceType) {
      ...
      const url = `/api/servicos/filtrar/tipo/${serviceType}`;
      this.httpClient.get(url, { headers }).subscribe(
        (response) => {
          this.responseData = response;
          if (!this.responseData || !this.responseData.servicos || this.responseData.servicos.length === 0) {
